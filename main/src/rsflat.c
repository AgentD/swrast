                                ...
                                const vec4 color, context* ctx )
{
    ...
    scale = (float)y0 - A.y;
    s->edge[0].v = vec4_add(s->edge[0].v, vec4_scale(s->edge[0].dvdy, scale));
    s->edge[1].v = vec4_add(s->edge[1].v, vec4_scale(s->edge[1].dvdy, scale));
...
void draw_triangle_flat( vec4 A, vec4 B, vec4 C, vec4 color, context* ctx )
{
    ...
    if( linescale[2] > 0.0f )
    {
        ...
        {
            temp[0] = B.y - A.y;
            s.edge[0].v = vec4_add( A, vec4_scale(s.edge[0].dvdy, temp[0]) );
        }
